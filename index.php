...
function mention($content) {
    ...
    foreach ($parts as $v) {
        if (0 === \strpos($v, '<') && '>' === \substr($v, -1)) {
            ...
            $out .= false !== \strpos($v, '@') ? \preg_replace_callback('/(?<=\W|^)@[a-z\d-]+/', static function ($m) use ($key) {
                if (\is_file($file = \LOT . \D . 'user' . \D . \substr($m[0], 1) . '.page')) {
                    $user = new \User($file);
                    if ('author' === $key) {
                        return '<a href="' . ($user->link ?? $user->url) . '" target="_blank" title="' . $user->user . '">' . $user . '</a>';
                    }
                    if ('user' === $key) {
                        return '<a href="' . ($user->link ?? $user->url) . '" target="_blank" title="' . $user . '">' . $user->user . '</a>';
                    }
                    if (\is_callable($key)) {
                        ...
                        if (\is_string($key) && null !== ($v = $user->{$key})) {
                            return '<a href="' . ($user->link ?? $user->url) . '" target="_blank" title="' . $user->user . '">' . $v . '</a>';
                        }
                        return \fire($key, [$m[0]], $user);
                    }
                    return '<a href="' . ($user->link ?? $user->url) . '" target="_blank" title="' . $user->user . '">' . ($user->{$key} ?? $user) . '</a>';
                }
                return $m[0];
            }, $v) : $v; // Is a plain text
        }
